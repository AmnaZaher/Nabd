...
import type { DoctorProfile, EditDoctorPayload } from '../../../api/profile';
import {
    Pencil,
    ShieldCheck,
    Clock,
...
// ==================== Form State ====================
// A flat form state that maps 1-to-1 with EditDoctorPayload field names
interface FormState {
    NameEngLish: string;
    NameArabic: string;
    Email: string;
    ...
    City: string;
    Country: string;
    // Read-only display fields (not sent to API)
    nationalId: string;
    avatar: string;
}

const buildFormState = (doctor: DoctorProfile): FormState => ({
    ...
    GraduationYear:           String(doctor.graduationYear    ?? ''),
    Specialization:           doctor.specialization           ?? '',
    MedicalSyndicateNumber:   doctor.medicalSyndicateNumber   ?? '',
...
const EditDoctorProfilePage = () => {
    ...
    const [form, setForm] = useState<FormState>({
        NameEngLish: '', NameArabic: '', Email: '', PhoneNumber: '',
        Gender: 'Male', DateOfBirth: '', ClinicId: '', IsActive: true,
        EducationalQualification: '', GraduationYear: '', Specialization: '',
        MedicalSyndicateNumber: '', Address: '', City: '', Country: '',
        nationalId: '', avatar: '',
    });
    ...
    useEffect(() => {
        const load = async () => {
            ...
            try {
                ...
                if (doctor) setForm(buildFormState(doctor));
            } catch (error) {
    ...
    const handleSave = async () => {
        ...
        setSaving(true);
        try {
            const payload: EditDoctorPayload = {
                ...
                MedicalSyndicateNumber:   form.MedicalSyndicateNumber   || undefined,
                ...
                <div className="max-w-[1400px] mx-auto pb-24">
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 lg:gap-8 items-start">
                            ...
                            <Card className="p-8 flex flex-col items-center text-center">
                                <div className="relative mb-6 group cursor-pointer">
                                    <div className="w-36 h-36 rounded-2xl overflow-hidden border-4 border-white shadow-xl bg-slate-100">
                                        <img
                                            src={form.avatar || `https://ui-avatars.com/api/?name=${encodeURIComponent(form.NameEngLish || 'D')}&background=random`}
                                            alt={form.NameEngLish}
                                    ...
                                    <div className={`absolute -bottom-2 -right-2 w-8 h-8 rounded-full border-4 border-white shadow-lg ${form.IsActive ? 'bg-emerald-500' : 'bg-slate-400'}`}></div>
                                </div>
                                <h2 className="text-xl font-black text-slate-900 mb-1">{form.NameEngLish || 'Doctor Name'}</h2>
                                <p className="text-slate-400 font-bold text-sm mb-4" dir="rtl">{form.NameArabic}</p>
                                <Badge variant="info" className="bg-blue-50 text-blue-600 border-0 font-black uppercase tracking-wide px-4 py-1.5 rounded-lg">
                            ...
                            <Card className="p-0 overflow-hidden">
                                ...
                                <div className="p-6 space-y-6">
                                    <div className="flex items-center justify-between">
                                        ...
                                        <div
                                            className={`w-12 h-6 rounded-full relative cursor-pointer transition-colors ${form.IsActive ? 'bg-blue-600' : 'bg-slate-300'}`}
                        ...
                        <div className="lg:col-span-8 xl:col-span-9 space-y-5">
                            ...
                            <FormSection title="Personal Information" icon={<Pencil size={18} />}>
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    <FieldGroup label="Full Name (English)">
                                        <Input name="NameEngLish" value={form.NameEngLish} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                    </FieldGroup>
                                    <FieldGroup label="Full Name (Arabic)">
                                        <Input name="NameArabic" value={form.NameArabic} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold text-right" dir="rtl" />
                                    </FieldGroup>
                                    <FieldGroup label="Email Address">
                                        <Input name="Email" value={form.Email} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                    </FieldGroup>
                                    <FieldGroup label="Phone Number">
                                        <Input name="PhoneNumber" value={form.PhoneNumber} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                    </FieldGroup>
                                    <FieldGroup label="Gender">
                                        <div className="relative">
                                            <select name="Gender" value={form.Gender} onChange={handleChange} className="w-full h-11 bg-slate-100/50 border border-slate-200 rounded-xl px-4 font-bold text-slate-900 appearance-none focus:outline-none focus:ring-2 focus:ring-blue-500/20">
                                    ...
                                    <FieldGroup label="National ID (Read-only)">
                                        <Input value={form.nationalId} readOnly className="bg-slate-100/30 border-slate-200 font-bold text-slate-400" />
                                    </FieldGroup>
                                    <FieldGroup label="Residential Address">
                                        <Input name="Address" value={form.Address} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                    </FieldGroup>
                                    <FieldGroup label="Date of Birth">
                                        <Input name="DateOfBirth" type="date" value={form.DateOfBirth} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                    </FieldGroup>
                                    <FieldGroup label="City">
                                        <Input name="City" value={form.City} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                    </FieldGroup>
                                    <FieldGroup label="Country">
                                        <Input name="Country" value={form.Country} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                    </FieldGroup>
                                </div>
                            </FormSection>
                            ...
                            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-start">
                                <div className="space-y-8">
                                    ...
                                    <FormSection title="ORGANIZATION ASSIGNMENT" icon={<Plus size={18} />}>
                                        <FieldGroup label="Clinic ID">
                                            <Input name="ClinicId" value={form.ClinicId} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" placeholder="e.g. 3" />
                                        </FieldGroup>
                                    </FormSection>
                                </div>
                                ...
                                <FormSection title="PROFESSIONAL QUALIFICATIONS" icon={<Plus size={18} />}>
                                    <div className="grid grid-cols-1 gap-6">
                                        <FieldGroup label="Specialization">
                                            <Input name="Specialization" value={form.Specialization} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                        </FieldGroup>
                                        <FieldGroup label="Educational Qualification">
                                            <Input name="EducationalQualification" value={form.EducationalQualification} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                        </FieldGroup>
                                        <div className="grid grid-cols-2 gap-4">
                                            <FieldGroup label="Graduation Year">
                                                <Input name="GraduationYear" value={form.GraduationYear} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" placeholder="e.g. 2010" />
                                            </FieldGroup>
                                            <FieldGroup label="Medical Syndicate No.">
                                                <Input name="MedicalSyndicateNumber" value={form.MedicalSyndicateNumber} onChange={handleChange} className="bg-slate-100/50 border-slate-200 font-bold" />
                                            </FieldGroup>
                                        </div>
                                    </div>
                                </FormSection>
                            ...
                            {/* Working Schedule (display-only in edit page; managed separately via schedule API) */}
